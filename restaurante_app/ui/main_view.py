...
class MainView(tk.Frame):
    def __init__(self, master, restaurante_servicio, usuario_actual, al_cerrar_sesion):
        super().__init__(master, bg="#fffaf0")
    ...
    def definir_estilos(self):
        ...
        self.color_fondo = "#fffaf0"
        self.color_encabezado = "#5a3410"
        self.color_texto = "#4a3216"
        self.color_secundario = "#fde3b8"
        self.color_resaltado = "#b45309"
        ...
        estilo.map("MenuApp.TButton", background=[("active", "#fad498")])
        estilo.configure(
        ...
        estilo.map("CerrarSesion.TButton", background=[("active", "#601818")])

    def construir_interfaz(self):
        # Construye la pantalla principal del sistema.
        ...
        tk.Label(
            encabezado,
            text=f"Bienvenido, {self.usuario_actual.nombre} (Turno: {self.usuario_actual.turno})",
            bg=self.color_encabezado,
            fg="#fde3b8",
            font=("Arial", 11),
        ...
        self.crear_boton_menu(barra, "Personal", self.mostrar_usuarios)
        self.crear_boton_menu(barra, "Turnos", self.mostrar_funcionalidad_pendiente)
    ...
    def mostrar_productos(self):
        ...
        for producto in self.restaurante_servicio.listar_productos():
            texto = (
                ...
                f"${producto.precio:.2f} | {producto.calorias} kcal | Stock: {producto.stock}"
            )
            self.crear_fila_informacion(texto)

    def mostrar_usuarios(self):
        # Solicita el personal al servicio y lo muestra en pantalla.
        self.limpiar_contenido()

        assert self.contenido is not None

        self.crear_titulo_seccion("Personal registrado")

        for usuario in self.restaurante_servicio.listar_usuarios():
            texto = f"{usuario.identificacion} - {usuario.nombre} | Turno: {usuario.turno}"
            self.crear_fila_informacion(texto)
    ...
    def crear_barra_estado(self):
        ...
        tk.Label(
            ...
            text=(
                ...
                f"Personal: {self.restaurante_servicio.cantidad_usuarios()} | "
